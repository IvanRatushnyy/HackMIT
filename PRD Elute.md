...
> **Amendment, Sept 20 (backend stage) — Elute may hold a grounded opinion.** Decided with the backend plan (`docs/BACKEND_PLAN.md` v4.4 §6). Elute may state its own opinion on a *research* candidate — a stance from a closed set (deprioritise · no clear prioritisation · pursue conditionally · insufficient evidence) — **only** downstream of the visible evidence and **always** shown with the supporting claims, the opposing claims, the unknowns, the weakest link, and what would change its mind. It may decline to prioritise. It never states a bare verdict, never fabricates a probability or score, never recommends prescribing, never reads as promotion, and never overrides the scientist's call, which stays a separate, attributed field. Where this document still says "the tool never recommends", read it as "never a bare or prescribing recommendation"; §6 step 6, §8, §10 and §12 are amended below to match.
...
6. **Trial prerequisites, then the decision.** Five conditions with a status each: brain exposure at tolerated doses (contested); target engagement measured in patients (unknown); benefit under blinding (negative, an outcome tag); biomarker validated against an alternative (contested); safety acceptable in the likely population (contested, with the boxed QT warning from the [FDA label](https://www.accessdata.fda.gov/drugsatfda_docs/label/2007/022068lbl.pdf) explained for an older, polypharmacy Parkinson's population). Then, beside it, **Elute's opinion**: a stance (deprioritise · no clear prioritisation · pursue conditionally · insufficient evidence) with the claims for, the claims against, the unknowns, and what would change it — grounded in the visible evidence, never a bare verdict, never forced. Then **Your call**: pursue / needs specific data / deprioritise, plus one line in the scientist's own words, recorded separately and attributed to them.
7. **Exit — the discussion deck.** *Export appraisal* produces a PowerPoint deck built to be put in front of colleagues: the questions a decision has to answer, each with the evidence for and against laid out with equal weight, the drawing, the prerequisites, the sources, and the data note. The scientist's own call is on its own page, attributed, and can be left out. The document presents; it does not argue (§7 states the neutrality rules).
...
The moment we're designing for: a judge looks at the drawing, says *"that's the c-Abl story, and the exposure step is the one that's dashed,"* and then asks for the deck.
...
flowchart LR
  ...
  D --> X[Exit<br/>PowerPoint discussion deck]
...
| **Won't** | A recommendation to prescribe, a bare verdict, or any opinion shown without its evidence beside it, on screen or in the export | Principle 7 as amended Sept 20: Elute may state a grounded opinion (stance + reasons + supporting/opposing claims + unknowns + what would change it) and may decline to prioritise; it never prescribes and never overrides the scientist's call. |
...
- **Not promotion.** It never recommends prescribing anything and never presents off-label use as endorsement. Its opinion on a *research* candidate (deprioritise · no clear prioritisation · pursue conditionally · insufficient evidence) is always downstream of visible evidence and shown with the case for, the case against, and the unknowns — never a bare verdict. Conclusions are at the drug-class or mechanism level wherever possible.
...
No, and it's designed not to be. It never recommends prescribing; it presents evidence and counter-evidence for a qualified scientist, and when it states its own research opinion the stance arrives with the claims for, the claims against, the unknowns, and what would change it. It doesn't mine real-world prescribing data (§8, Won't). Language throughout is "here's the case, the case against, and where we lean and why — you decide."
