...
| Raspberry | `#82204a` | 130 32 74 | `oklch(41.5% 0.137 359)` | "Single-source", "refuted", the safety flag, the negative outcome chip, destructive actions | rare |
...
| Refuted | Raspberry | The word "refuted", a solid link: tested directly in a blinded controlled study and found false |

Every label is an 8px square in its color followed by the word. Never convey a label by color alone. A label is never bare: the source, or the rule that produced it, is one click away.

The safety flag is a raspberry word with its reason next to it, never a banner. A *negative* outcome chip is raspberry-outlined. What set a rank is a driver bar of four named segments with monochrome pips, never a number.
